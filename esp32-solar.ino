...
TFT_eSprite sunSprite = TFT_eSprite(&tft);
...
void setupSprites() {
  // Sun
  sunSprite.setColorDepth(8);
  sunSprite.createSprite(SUN_WIDTH, SUN_HEIGHT);
  sunSprite.fillSprite(TFT_TRANSPARENT);
  sunSprite.pushImage(0, 0, SUN_WIDTH, SUN_HEIGHT,  (uint16_t *)sunImage, 8);
}
...
void loop() {
  ...
  sunSprite.pushSprite(SUN_X - (SUN_WIDTH / 2), SUN_Y - (SUN_HEIGHT / 2), 0x00);
